...
static void my_application_activate(GApplication* application) {
  ...
  // Resolve the bundled icon relative to the running executable so this works
  // both from Flutter's build bundle and from relocatable Daymark archives.
  g_autofree gchar* executable_path = g_file_read_link("/proc/self/exe", nullptr);
  if (executable_path != nullptr) {
    g_autofree gchar* executable_dir = g_path_get_dirname(executable_path);
    g_autofree gchar* icon_path =
        g_build_filename(executable_dir, "data", "daymark-icon.png", nullptr);
    g_autoptr(GError) icon_error = nullptr;
    if (!gtk_window_set_icon_from_file(window, icon_path, &icon_error)) {
      g_warning("Daymark window icon unavailable: %s", icon_error->message);
    }
  }
