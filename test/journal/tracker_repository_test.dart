...
void main() {
  ...
  test(
    'rejects a sixth overlapping Tracker and reuses a free slot later',
    () async {
      for (int index = 0; index < 5; index++) {
        ...
        final TrackerRecord record = (await repository.loadForDay('2026-09-15'))
  ...
  test(
    'ending early preserves earlier history and removes later marks',
    () async {
      final String trackerId = await repository.create(
        title: 'Read',
        startDate: '2026-09-01',
      ...
      await repository.endEarly(trackerId: trackerId, methodDate: '2026-09-15');

      final TrackerMonthSnapshot month = await repository.loadMonth(
        '2026-09-01',
      );
  ...
  test(
    'month snapshot includes only Trackers intersecting the month',
    () async {
