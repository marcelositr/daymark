...
void main() {
  ...
  test('JSON export is deterministic and preserves journal relationships', () async {
    ...
    final Map<String, Object?> migration = _maps(
      payload['migrations'],
    ).single;
    ...
    final Map<String, Object?> indexItem = _maps(payload['indexItems']).single;
    expect(indexItem['collectionId'], 'collection-1');
  });

  test('Markdown export is plaintext, readable, and preserves Unicode', () async {
    await _seedExportFixture(database);

    final OpenExportDocument document = await OpenExportService(
      database,
