...
///
/// Reads query the encrypted database directly. Mutations delegate to the
/// existing application service so entry ownership and task-state invariants
/// remain centralized in the journal repository layer.
final class DailyLogRepository {
  ...
  Future<DailyLogSnapshot> loadOrCreate(String methodDate) async {
    _validateMethodDate(methodDate);
    ...
    if (logId == null) {
      try {
        ...
        // A concurrent refresh may have created the same Daily Log after the
        // read above. Re-read once; any other invariant error remains visible.
    ...
    return DailyLogSnapshot(
      ...
      entries: await _loadEntries(resolvedLogId),
    );
  }
...
void _validateMethodDate(String value) {
